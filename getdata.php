<?
	...
	//接收書的ID以及問題ID
	//$this_book_id = $_POST["book_id"];
	//$this_question_no = $_POST["question"];

	$this_book_id = $number_quiz_sql->book_id;
	...
	//取得此題的答案數量
	$sql_number_quiz = "select count(question_id) AS Count from Question where book_id like '".$this_book_id."' and question_no like '".$this_question_no."'";	
	...
	$sql_checkbox = "select * from Question where book_id like '".$this_book_id."' and question_no like '".$this_question_no."' and QA like 'Q'";
	...
	*/
	...
	if($number_quiz == 5){
		...
		if($exam_type == 'SINGLE'){
			for( $i = 1 ; $i <= 4 ; $i++){
				$sql_data = "select * FROM Question WHERE book_id like '".$this_book_id."' AND question_no like '".$this_question_no."' AND QA like 'A".$i."'";
